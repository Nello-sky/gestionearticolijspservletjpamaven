...
public class PrepareDeleteArticoloServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String idArticoloToDelete = request.getParameter("idArticolo");

		if (!NumberUtils.isCreatable(idArticoloToDelete)) {
		...
		try {
			request.setAttribute("ArticoloToDelete", MyServiceFactory.getArticoloServiceInstance()
					.caricaSingoloElemento(Long.parseLong(idArticoloToDelete)));
		} catch (Exception e) {
			// qui ci andrebbe un messaggio nei file di log costruito ad hoc se fosse attivo
			e.printStackTrace();
			request.setAttribute("errorMessage", "Attenzione si è verificato un errore.");
